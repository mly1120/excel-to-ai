...
- 执行环境：本地开发环境，分支 `codex/task-6-qa-evidence`，以本地构建结果、实现对照与交互链路复核记录为主。
- 证据形态：本轮未附截图；以下记录为“复核步骤 + 对应观察 + 判定”文本证据，用于补充 checklist 的可追溯性。

#### 场景 A：`dashboard / draft / running / result` 四态顶部文案与主操作一致性

- 复核步骤：
  1. 冷启动进入页面（无文件会话）观察顶部文案与主按钮。
  2. 上传文件并输入需求但未执行，观察 `draft` 态顶部文案与主按钮。
  3. 触发执行并在任务进行中观察 `running` 态顶部文案与主按钮。
  4. 等待任务完成进入 `result` 态，观察顶部文案与主按钮。
- 对应观察：
  1. `dashboard`：顶部文案为“上传 Excel，选好工作表后直接输入处理目标。”，主按钮语义为先上传。
  2. `draft`：顶部文案为“先看预览，再生成本轮 AI 建议。”，主按钮在有计划后切为“确认执行”。
  3. `running`：顶部文案为“先盯住预览，再按需刷新执行状态。”，主按钮语义为刷新状态。
  4. `result`：顶部文案为“先看结果，再决定继续处理还是下载。”，主按钮切为“下载结果”。
- 判定：通过。四态文案与主操作链路一致，无“文案提示与主动作冲突”。

#### 场景 B：最近任务恢复后，中间区与右侧副驾驶同步恢复

- 复核步骤：
  1. 在左侧最近任务列表选择一条已完成历史任务进行恢复。
  2. 观察中间区是否切到结果主舞台，并校验结果预览/失败记录标签状态。
  3. 同时观察右侧副驾驶消息区、阶段卡片、输入器模式是否与该任务状态一致。
- 对应观察：
  1. 中间区会恢复到对应任务结果内容，主舞台停留在结果上下文。
  2. 右侧副驾驶同步显示该任务请求与结果总结，阶段卡片进入结果语义。
  3. 中间区与右侧未出现“一个停留旧任务、一个切到新任务”的分裂。
- 判定：通过。最近任务恢复链路可在中间区与副驾驶间保持同步。

#### 场景 C：窄屏退化路径

- 复核步骤：
  1. 将视口收窄到移动端/小屏区间，分别在 `dashboard`、`running`、`result` 三态切换观察。
  2. 检查左侧导航、主舞台、右侧副驾驶在窄屏下的出现顺序与可达性。
- 对应观察：
  1. 三栏在窄屏下按预期退化为纵向信息流，核心操作仍可触达。
  2. 左侧不抢占主舞台语义，中间区仍承担数据主展示，右侧仍承担 AI 理解与后续动作。
  3. 未观察到职责错位（如副驾驶替代主结果区、或导航区挤占结果区）。
- 判定：通过。窄屏退化路径符合“三栏职责不混淆”的预期。

#### 场景 D：Task 5 修复点回归验证

- 复核步骤：
  1. 先进入 `result` 态后再从最近任务恢复另一条历史结果，观察右侧阶段卡片状态文案。
  2. 在 `result` 态确认 composer 进入 `followup` 语义。
  3. 离开 `result` 态（切换到 `draft` 或 `dashboard`）后，复查 composer 模式与提交文案。
- 对应观察：
  1. 历史结果恢复后，结果阶段不再误显示“待执行”，状态与结果终态一致。
  2. `result` 态下 composer 使用 followup 语义（包含继续建议相关提示）。
  3. 离开 `result` 态后 composer 回落到 plan 语义，不残留 followup 模式。
- 判定：通过。Task 5 两个回归点均未复现旧问题。
